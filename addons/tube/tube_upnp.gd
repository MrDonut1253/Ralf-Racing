...
func delete_port_mapping(p_public_port: int):
	...
	# Wenn UPnP nicht bereit ist, brauchen wir auch keinen Task starten
	if not is_port_mapping_ready:
		return
...
func _notification(what):
	if what == NOTIFICATION_PREDELETE:
		# 1. Erst auf alle laufenden Tasks (Init, Mapping) warten
		for id in task_ids:
			WorkerThreadPool.wait_for_task_completion(id)
		task_ids.clear()
		
		# 2. Wenn UPnP fehlgeschlagen ist (is_port_mapping_ready == false),
		# dürfen wir NICHTS tun. Die 'mapped_ports' sind nur da, weil wir sie 
		# optimistisch hinzugefügt haben. Wir löschen die Listen und gehen.
		...
		# 3. Wenn UPnP läuft, löschen wir die Ports SYNCHRON.
		# Threads im Destruktor zu starten ist gefährlich (Race Conditions).
		# Da das Spiel eh beendet wird, ist ein kurzer Blocking-Call okay.
		for port in mapped_ports:
			if upnp: 
